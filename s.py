...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7682560871:AAHIrO7wpPuzUe52k_Q1nff3IH8W4cXJK2M'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ YE BOT FREE NAHI H JISE LENA H BUY KARO @RAJPUTDDOS❌!!*", parse_mode='Markdown')
        return

    message = (
        "*🔥 Welcome to the Rajput Ddos! 🔥*\n\n"
        "* PHELE BOT BUY KAR LE FIR /bgmi DAL KE IP PORT TIME LIKHNA H *\n"
    )
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
      ...
      f"🎯 𝗧𝗔𝗥𝗚𝗘𝗧: {ip}:{port}\n"
      f" THANKS FOR USING RAJPUT DDOS\n", parse_mode='Markdown')
